...
export function loadGoogleCredentials() {
  try {
    ...
    if (process.env.GOOGLE_CREDENTIALS) {
      console.log('📡 Cargando credenciales desde variable de entorno GOOGLE_CREDENTIALS...');
    ...
    if (!credentialsPath) {
      throw new Error(
        'No se encontraron credenciales.\n' +
          'Configura GOOGLE_CREDENTIALS (JSON completo) o GA4_CREDENTIALS_PATH (ruta al archivo)',
      );
    }
    ...
    if (!fs.existsSync(absolutePath)) {
      throw new Error(
        `Archivo de credenciales no encontrado: ${absolutePath}\n` +
          `Tip: Configura GOOGLE_CREDENTIALS como variable de entorno con el JSON completo`,
      );
    }

    const credentials = JSON.parse(fs.readFileSync(absolutePath, 'utf8'));
    console.log('✅ Credenciales de Google cargadas desde archivo:', absolutePath);
    return credentials;
  } catch (error) {
